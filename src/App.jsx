...
import {
  ...
  Sparkles, Loader2, Info, AlertCircle
} from 'lucide-react';
...
const T = {
  zh_TW: {
    ...
    lastUpdateBy: '最後更新時間：',
    adLabel: '贊助商內容',
    aiStrategyBtn: '✨ 生成 AI 智能分析',
    aiLoading: 'AI 正在計算最佳回報方案...',
    aiTitle: 'AI 投資策略助手'
  },
  en: {
    title: 'HK FD Tracker',
    ...
    aiStrategyBtn: '✨ Get AI Strategy',
    aiLoading: 'AI computing best yields...',
...
export default function App() {
  ...
  // Gemini AI Strategy Integration
  const fetchAiStrategy = async () => {
    setIsAiLoading(true);
    setAiError("");
    setAiResponse("");

    const top3 = sortedBanks
      .filter(b => b.rates?.HKD?.[tenor] > 0)
      .slice(0, 3)
      .map(b => `${b.name[lK]}: ${b.rates.HKD[tenor]}% (Min: HK$${b.minDeposit})`)
      .join(", ");

    const prompt = `
      As a professional Hong Kong financial advisor, provide a short investment strategy for a user with HK$${amount.toLocaleString()} for a ${tenor} tenor.
      Current top 3 banks and rates are: ${top3}.
      User language preference: ${lang === 'zh_TW' ? 'Traditional Chinese' : 'English'}.
      Focus on diversification, yield optimization, and liquidity needs. Keep the tone professional but accessible.
    `;
    ...
    try {
      ...
      setAiError("AI Assistant temporarily unavailable. Please try again later.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans antialiased pb-20 selection:bg-blue-100">
      {/* Top Navbar */}
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-xl">
            {['zh_TW', 'en'].map(l => (
              <button key={l} onClick={() => setLang(l)} className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${lang === l ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}>
                {l === 'zh_TW' ? '繁' : 'EN'}
              </button>
            ))}
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-8">
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-9 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-wrap gap-8 items-end">
              ...
              <div className="space-y-2 w-full md:w-auto">
                ...
                <div className="flex bg-slate-50 p-1 rounded-2xl gap-1">
                  {['3m', '6m', '12m'].map(m => (
                    <button key={m} onClick={() => setTenor(m)} className={`px-8 py-2.5 rounded-xl text-sm font-black transition-all ${tenor === m ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:bg-slate-200'}`}>
                      {m.toUpperCase()}
                    </button>
                  ))}
                </div>
            ...
            {/* AI Mobile Trigger (Visible only on Mobile) */}
            <button onClick={fetchAiStrategy} disabled={isAiLoading} className="lg:hidden w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl py-4 font-black flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform">
              {isAiLoading ? <Loader2 className="animate-spin" /> : <Sparkles size={18} />}
              {isAiLoading ? t.aiLoading : t.aiStrategyBtn}
            </button>
                ...
                return (
                  <div key={bank.id} className={`group bg-white rounded-2xl border border-slate-200 p-4 flex flex-wrap items-center gap-4 transition-all hover:shadow-lg hover:border-blue-200 ${belowMin ? 'opacity-40 grayscale' : ''}`}>
                    ...
                    <div className="flex items-center gap-4 min-w-[320px] flex-1">
                      <div className="w-12 h-12 rounded-xl bg-slate-50 p-2 border border-slate-100 flex items-center justify-center shrink-0 shadow-inner">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase ${bank.fundType === 'new' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
                    ...
                    <div className="flex items-center gap-8 md:gap-12 ml-auto">
                      <div className="text-right">
                        ...
                        <p className={`text-3xl font-black tabular-nums leading-none ${hasR ? 'text-slate-900' : 'text-slate-100'}`}>{hasR ? `${r.toFixed(3)}%` : '--'}</p>
                      </div>
            ...
            <div className="bg-white border border-dashed border-slate-300 rounded-3xl p-4 flex items-center justify-center min-h-[120px]">
              <div className="text-center">
                ...
                <div className="w-full h-full bg-slate-50 rounded flex items-center justify-center border border-slate-100">
                   <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest">Banner Ad Zone</span>
                </div>
              </div>
            </div>
            ...
            <footer className="mt-12 p-10 bg-white rounded-3xl border border-slate-200 space-y-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-slate-900 rotate-12"><ShieldCheck size={160} /></div>
              <div className="flex items-center gap-3 text-slate-900 font-black uppercase tracking-widest text-sm"><ShieldCheck size={20} className="text-blue-500" /> Compliance & Accuracy</div>
              <div className="grid md:grid-cols-2 gap-8 text-xs font-bold text-slate-400 leading-relaxed uppercase">
                ...
                <p>• {lang === 'zh_TW' ? '本系統僅提供利率參考，最終年利率及條款以銀行辦理時為準。' : 'Rates provided are for reference only. Final rates and terms are subject to bank approval.'}</p>
              </div>
              <div className="pt-6 border-t border-slate-100 flex justify-between items-center text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">
                <span>HK FD Tracker Pro</span>
                <span>Version 2.5 • AI Enhanced</span>
              </div>
            </footer>
          </div>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-slate-900 rounded-3xl p-6 text-white shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-center gap-2">
                    <div className="bg-blue-600 p-1.5 rounded-lg"><Sparkles size={16} /></div>
                    <h4 className="font-black text-sm uppercase tracking-widest">{t.aiTitle}</h4>
                  </div>
                  
                  {aiResponse ? (
                    <div className="bg-slate-800/50 rounded-2xl p-4 text-[11px] font-medium leading-relaxed border border-slate-700 max-h-[400px] overflow-y-auto custom-scrollbar">
                      {aiResponse.split('\n').map((line, i) => <p key={i} className="mb-2">{line}</p>)}
                    </div>
                  ) : (
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tighter italic">
                      {isAiLoading ? t.aiLoading : "Click below for AI-powered wealth allocation strategy."}
                    </p>
                  )}
                  ...
                  <button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-xl py-3 text-xs font-black transition-all flex items-center justify-center gap-2 shadow-lg"
                  >
                    {isAiLoading ? <Loader2 className="animate-spin w-4 h-4" /> : <Sparkles size={14} />}
                    {isAiLoading ? t.loading : t.aiStrategyBtn}
                  </button>
                </div>
              </div>

              {/* Sidebar Ad Slot */}
              <div className="bg-white border border-dashed border-slate-300 rounded-3xl p-4 flex flex-col items-center min-h-[500px] shadow-sm">
                ...
                <div className="w-full flex-1 bg-slate-50 rounded border border-slate-100 flex items-center justify-center text-center p-4">
                  <span className="text-[10px] text-slate-300 font-bold leading-relaxed uppercase tracking-widest">
                    Skyscraper Ad Slot<br/>(300x600)
                  </span>
                </div>
              </div>
